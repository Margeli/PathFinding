...
void j1Map::Path_to(iPoint origin, iPoint destination) {

	ResetPath();

	iPoint goal = WorldToMap(destination.x, destination.y);
	iPoint start = WorldToMap(origin.x, origin.y);
	if (MovementCost(goal.x, goal.y) >= 0) {// Check is walkable
...
int j1Map::MovementCost(int x, int y) const // returns -1 if out of map
{
	int ret = -1;
...
void j1Map::PropagateBFS()
{
	...
	if (frontier.Pop(curr))
	{
		...
		for (uint i = 0; i < 4; ++i)
		{
			if (MovementCost(neighbors[i].x, neighbors[i].y) > 0) // check if its walkable
			{
				if (visited.find(neighbors[i]) == -1)
				{
